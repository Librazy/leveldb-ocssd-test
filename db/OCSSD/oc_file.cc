...
leveldb::Status oc_file::Flush()
{
}

oc_file::Node* oc_file::AllocNode(int degree)
{
	oc_file::Node* ptr = (oc_file::Node*) malloc(sizeof(oc_file::Node) + (degree - 1) * sizeof(oc_file::StripeDescriptor*));
	if (!ptr) {
		s = leveldb::Status::IOError("oc_file::AllocNode", strerror(errno));
		return NULL;
	}
	ptr->degree = degree;
	ptr->size = 0;
