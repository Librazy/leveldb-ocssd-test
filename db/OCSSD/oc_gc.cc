...
static inline void setupaddrs_pl_blks(size_t pl_blk_st, struct nvm_addr *addrs, size_t pl_blks_num, size_t pl_num)
{
...
leveldb::Status oc_GC::EraseByLun(int ch, int lun, oc_block_manager *blk_mng)
{
	...
	if (!valid_lun(lun, blk_mng->geo_)) {
		s = leveldb::Status::IOError(GCErrMsg("EraseByLun: Invalid Lun:", lun));
		goto OUT;
	}
	...
	while (erased_pl_blks < remained_pl_blks) {	
		...
		if (nvm_addr_erase(blk_mng->ssd_->dev_, addrs, emit_blks_num, blk_mng->ssd_->pmode_, &ret)) {
			s = leveldb::Status::IOError(GCErrMsg("EraseByLun, do erase:", lun, pl_blk_st, pl_blk_st + emit_pl_blks_num - 1), strerror(errno));
			goto OUT;
		}
...
leveldb::Status oc_GC::EraseByBlock(int ch, int lun, int blk, oc_block_manager *blk_mng)
{
	leveldb::Status s;
